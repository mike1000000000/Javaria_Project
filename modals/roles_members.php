 ...
 *
 * File: roles_members.php
 * Last Modified: 8/24/19, 12:53 PM
 */

echo call_user_func(function () {
    $modalwriter = new htmlwriter();
    $modalwriter->modalid        = 'modal_rolesmembers';
    $modalwriter->labelsize      = 'col-md-4';
    $modalwriter->inputsize      = 'col-md-6';
    $modalwriter->title          = mlang_str('MODAL-ROLES_MEMBERS_TITLE', true);
    $modalwriter->footercancel   = mlang_str('CANCEL', true);
    $modalwriter->footeraccept   = mlang_str('UPDATE', true);
    $modalwriter->modalsize      = 'modal-';
    $modalwriter->acceptcustomfunction = 'rolesmemberupdate()';
    $modalwriter->includescript = '/modals_js/roles_members.js';

    $formcontent  = $modalwriter->createFormTextInput('MODAL-ROLES_MEMBERS_TXT_NAME','name',null,array('disabled'=>''));
    $formcontent .= $modalwriter->createLine();

    $label1 = $modalwriter->createLabel('MODAL-ROLES_MEMBERS_LBL_USERS', '', array('labelsize'=>''), array('style'=>'margin-left:5px; text-align:left;'));
    $label2 = $modalwriter->createLabel('MODAL-ROLES_MEMBERS_LBL_SELECTEDUSERS', '', array('labelsize'=>''), array('style'=>'margin-left:10px; text-align:left;'));
    ...
    $selectbox = $modalwriter->createSelectBasic( 'selectroleusers',null, array('multiple'=>'multiple'));
